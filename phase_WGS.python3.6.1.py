...
count=len(arguments)
if count !=6:
	print ("Usage: python phase_WGS.py sample_ID bam_dir data_dir ref_fasta n_jobs input_positions(file format:chr pos ref alt, sep=\\t)")
#1       13799   T       G       0/1     236     8       0.033   5       3       0.625   8447    282     146     90
	sys.exit(1)
elif count==6:
	sample=sys.argv[1]
	bam_dir_tmp=sys.argv[2]
	data_dir_tmp=sys.argv[3]
	ref_fasta=sys.argv[4]
	n_jobs=sys.argv[5]
	input_pos=sys.argv[6]
	log_file=sample+'multiple_inforSNPs.log'
...
for line in file:
	...
	chrom=str(chr)
	pos=int(fields[1])
	major_allele=fields[2]
	minor_allele=fields[3]
	...
	print (chr,pos,major_allele,minor_allele,major_num,minor_num,conflictnum,file=fo1)
file.close()
fo1.close()
...
file=open(sample+"_candidates")
merged_inforSNPs=open(sample+".merged.inforSNPs.pos","w")
...
for line in file:
	...
	major_allele=fields[2]
	minor_allele=fields[3]
	major_num=int(fields[4])
	minor_num=int(fields[5])
	conflictnum=int(fields[6])
	...
	if major_num>=3 and minor_num>=3:
		...
		for rec in pysamstats.stat_variation(f3_alignment_file, fafile=ref_fasta, min_mapq=0, min_baseq=0):
			if  ([rec['A'],rec['C'],rec['G'],rec['T']].count(0)<=2) and (rec['reads_pp']>10):
				...
				if alt_num>=5 and max_num>=5 and scipy.stats.binom_test(alt_num, max_num+alt_num)>0.05 and rec['pos']!=int(pos):
					print (chr,pos,major_allele,minor_allele,chr,int(rec['pos'])+1,max_base,alt_base,conflictnum, file=merged_inforSNPs)
...
input_pos=sample+".merged.inforSNPs.pos"
...
for line in file:
	...
	major_allele=fields[2]
	minor_allele=fields[3]
	inforSNP_pos=fields[5]
	inforSNP_ref=fields[6]
	inforSNP_alt=fields[7]
	conflict=fields[8]
	if pos != inforSNP_pos:
		f1_sorted_name=output_dir+"/"+sample+"."+str(chr)+"_"+str(pos)+".mosaic.major.sorted.bam"
		f2_sorted_name=output_dir+"/"+sample+"."+str(chr)+"_"+str(pos)+".mosaic.minor.sorted.bam"
		...
		name=chr+'_'+str(pos)+'_'+major_allele+"_"+minor_allele+"_"+str(inforSNP_pos)+"_"+str(inforSNP_ref)+"_"+str(inforSNP_alt)+"_"+str(conflict)
		n_major_het1[name]=0
		n_major_het2[name]=0
		n_minor_het1[name]=0
		n_minor_het2[name]=0
...
for k,v in sorted(n_major_het1.items()):
	...
	print (sample,' '.join(str(x) for x in k.split("_")), n_major_het1[k],n_major_het2[k],n_minor_het1[k],n_minor_het2[k],file=fo2)
fo2.close()
